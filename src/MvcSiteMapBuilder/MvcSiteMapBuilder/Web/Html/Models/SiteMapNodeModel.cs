...
namespace MvcSiteMapBuilder.Web.Html.Models
{
        ...
        : ISortable
    {
        ...
        public SiteMapNodeModel(SiteMap siteMap, SiteMapNode node, IDictionary<string, object> sourceMetadata)
            : this(siteMap, node, sourceMetadata, Int32.MaxValue, true, false, true)
        {
        }
        ...
        protected readonly SiteMapNode node;
                    ...
                    if (ReachedMaximalNodelevel(maxDepth, node, drillDownToCurrent) && node.HasChildNodes)
                    {
                        ...
                        if (VisibilityAffectsDescendants)
                        {
                            foreach (var child in sortedNodes)
                            {
                                ...
                                {
                                    children.Add(new SiteMapNodeModel(siteMap, child, SourceMetadata, maxDepth - 1, drillDownToCurrent, false, VisibilityAffectsDescendants));
                                }
                            }
                        }
                        else
        ...
        private void FindNearestVisibleDescendants(SiteMapNode node, int maxDepth, ref List<SiteMapNodeModel> nearestVisibleDescendants)
        {
            foreach (var child in node.ChildNodes)
            {
                if (child.IsAccessibleToUser())
                {
                    ...
                    {
                        nearestVisibleDescendants.Add(new SiteMapNodeModel(siteMap, child, SourceMetadata, maxDepth - 1, drillDownToCurrent, false, VisibilityAffectsDescendants));
                    }
                    else if (maxDepth > 1) //maxDepth should be greater then 1 to be allowed to descent another level
                    {
                        FindNearestVisibleDescendants(child, maxDepth - 1, ref nearestVisibleDescendants);
        ...
        public SiteMapNodeModel Parent
        {
            get
            {
                var parentNode = node.GetParentNode(siteMap);
                return parentNode == null
                    ? null
                    : new SiteMapNodeModel(siteMap, parentNode, SourceMetadata, maxDepth == Int32.MaxValue ? Int32.MaxValue : maxDepth + 1, drillDownToCurrent, startingNodeInChildLevel, VisibilityAffectsDescendants);
            }
        }
